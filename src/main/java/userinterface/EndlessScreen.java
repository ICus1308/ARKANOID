...
public class EndlessScreen extends UIManager implements GameScreen {
    private int lives = 3;
    private int score = 0;
    private int level = 1;
    ...
    public EndlessScreen(Pane root, CoinManager coinManager) {
        super(root);
    ...
    protected void initializeUI() {
        ...
        coinsText = createStyledText("Coins: " + (coinManager != null ? coinManager.getCoins() : 0),
                GAME_WIDTH / 2 - 150, 25, UI_FONT, GOLD_COLOR);
        levelText = createStyledText("Level: 1", GAME_WIDTH / 2 + 50, 25, UI_FONT, TEXT_COLOR);
        gameMessage = createStyledText("PRESS SPACE TO START", GAME_WIDTH / 2 - 200, GAME_HEIGHT / 2, MESSAGE_FONT, GOLD_COLOR);

        root.getChildren().addAll(scoreText, livesText, coinsText, levelText, gameMessage);
    }
    ...
    public void cleanup() {
        if (root != null) {
            root.getChildren().removeAll(scoreText, livesText, coinsText, levelText, gameMessage);
        }
    }
}
